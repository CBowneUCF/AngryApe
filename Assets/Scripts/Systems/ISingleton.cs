...
/// <summary>
/// A type of Behavior that can only exist once. <br/>
...
public interface ISingleton<T> : Singleton where T : UnityEngine.Object, ISingleton<T>
{
    ...
    public static T FromPreloaded()
    {
        if (typeof(T).ImplementsOrDerives(typeof(Component)))
        {
            if (Singleton.GetPrefab<T>(out GameObject result))
            {
                var resultI = GameObject.Instantiate(result);
                return resultI.GetComponent<T>();
            }
            else throw new Exception("No Saved Prefab Found.");
        }
        else throw new Exception("This is not a type that can have a prefab.");
    }
...
public interface Singleton
{
    ...
    {
        //Check if this actually works later.
        GameObject[] preloads = Resources.FindObjectsOfTypeAll<GameObject>();
        for (int i = 0; i < preloads.Length; i++)
            if (preloads[i].TryGetComponent(out Singleton result)) 
                prefabs.Add(result.GetType(), preloads[i]);
    }
